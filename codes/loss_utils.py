"""
I don't know where I got this code - maybe 2DPASS? But this was from other code and I was following the existing loss function
"""
import torch
...
seg_num_per_class = np.array([0, 55437630, 320797, 541736, 2578735, 3274484, 552662, 184064, 78858, 240942562, 17294618, 170599734, 6369672, 230413074, 101130274, 476491114, 9833174, 129609852, 4506626, 1168181])
seg_labelweights = seg_num_per_class / np.sum(seg_num_per_class)
seg_labelweights = torch.Tensor(np.power(np.amax(seg_labelweights) / seg_labelweights, 1 / 3.0))
seg_labelweights[0] = 0

ce_loss = nn.CrossEntropyLoss(weight=seg_labelweights, ignore_index=0).cuda()
lovasz_loss = Lovasz_loss(ignore=0).cuda()

def calc_loss(logits, labels):
    loss1 = ce_loss(logits, labels)
    loss2 = lovasz_loss(torch.nn.functional.softmax(logits, dim=1), labels)
    return loss1 + loss2